...
function HomeScreen({
  ...
  const practiceModes = [
    {
      label: "Paraphrase",
      desc: "Restate the text in your own words",
      Icon: MessageCircle,
    },
    { label: "Cold Call", desc: "Answer without seeing the text", Icon: Zap },
    {
      label: "Stand & Deliver",
      desc: "Full recitation performance",
      Icon: Mic,
    },
  ];
  return (
    ...
    >
      <div className="px-5 pt-3 flex items-center justify-between">
        <div>
          ...
          <h2
            className="text-xl font-black text-[#4B4032]"
            style={{ fontFamily: "Fraunces, serif" }}
          >
            {userName}
          </h2>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            onClick={onProfile}
            className="w-10 h-10 rounded-2xl bg-[#D6B15E] flex items-center justify-center text-white font-black text-[11px] shadow-sm active:scale-95 transition-transform"
          >
            {initials}
          </button>
        </div>
      ...
      <div className="mx-5 mt-4 bg-gradient-to-br from-[#D6B15E] to-[#BF9840] rounded-3xl p-5 shadow-lg shadow-[#D6B15E]/25 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-white/65 text-[10px] font-semibold uppercase tracking-wide">
                Total Points
              </p>
              <p
      ...
      <div className="mx-5 mt-3.5 grid grid-cols-3 gap-2.5">
        {[
          ...
          <div
            key={m.label}
            className="bg-white rounded-2xl p-3.5 border border-[#E7D3A8]/60 shadow-sm"
          >
            <p className="text-[9px] text-[#7A736B] font-bold uppercase tracking-wide mb-1">
              {m.label}
            </p>
            <div className="flex items-baseline gap-0.5">
              <span className="text-xl font-black" style={{ color: m.c }}>
                {m.value}
              </span>
              <span className="text-[10px] text-[#7A736B] font-medium">
                {m.unit}
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2">
            {[...history]
              ...
              .map((h, i) => (
                <div
                  key={i}
                  className="bg-white rounded-2xl px-4 py-3 flex items-center gap-3 border border-[#E7D3A8]/60 shadow-sm"
                >
                  ...
                  <p className="text-[#4B4032] text-xs font-medium flex-1 truncate">
                    {h.feedback.slice(0, 50)}
                    {h.feedback.length > 50 ? "…" : ""}
                  </p>
                  <span className="text-sm font-black text-[#A8CFA0] shrink-0">
                    {Math.round(
                      (h.scores.accuracy +
                        h.scores.confidence +
                        h.scores.clarity) /
          ...
          {showLanding ? (
            ...
            <>
              <div className="flex-1 overflow-hidden min-h-0">
                ...
                {activeTab === "sessions" && (
                  <SessionsScreen history={history} />
                )}
                {activeTab === "profile" && (
                  <ProfileScreen
                    userName={userName}
